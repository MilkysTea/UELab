...
namespace Touch {
        ...
        while (initialized) {
            ...
            for (size_t j = 0; j < count; j++) {
                ...
                if (ie.code == ABS_MT_TRACKING_ID) {
                    ...
                    device.Finger[latest].id = (i * 2 + 1) * maxF + ie.value;
                    continue;
                }
                ...
                if (ie.code == SYN_REPORT) {
                    Upload();
                    memset(&device.Finger[i], 0, sizeof(device.Finger) * 10);
                    latest = -1;
                    continue;
                }
            }
    ...
    void SleepDown(float x, float y,float AimSlepp) {
        lock.lock();
        touchObj &touch = devices[0].Finger[9];
        touch.id = 19;
        touch.pos = Vector2(x, y) * touch_scale;
        touch.isDown = true;
        usleep(AimSlepp * 1000);
        Upload();
        lock.unlock();
    }
